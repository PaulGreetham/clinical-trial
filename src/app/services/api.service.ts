...
export class ApiService {
  ...
  getRandomTrials(count: number = 10): Observable<Trial[]> {
    const params = this.getBaseParams(count).set(
      'filter.overallStatus',
      ['RECRUITING', 'ACTIVE_NOT_RECRUITING'].join(',')
    );
  ...
  getRandomSingleTrial(): Observable<Trial[]> {
    ...
    return this.executeTrialsRequest(params, 'Failed to fetch single trial').pipe(retry(2));
  }
  ...
  private getBaseParams(pageSize: number = 10): HttpParams {
    return new HttpParams()
      ...
      .set(
        'fields',
        'NCTId,BriefTitle,BriefSummary,DetailedDescription,Phase,OverallStatus,StartDate'
      );
  }

  private executeTrialsRequest(params: HttpParams, errorMessage: string): Observable<Trial[]> {
    return this.http
      .get<ClinicalTrialsApiResponse>(`${this.apiUrl}/studies`, {
        params,
        headers: {
  ...
  private handleApiError(error: Error | unknown, operation: string): Observable<never> {
    console.error(`${operation}: ${error instanceof Error ? error.message : String(error)}`);
    return throwError(
      () => new Error(`${operation}: ${error instanceof Error ? error.message : String(error)}`)
    );
  }
  ...
  private mapStudiesToTrials(studies: Array<ClinicalTrialStudy>): Trial[] {
    return studies.map(study => this.mapStudyToTrial(study));
  }

  private mapStudyToTrial(study: ClinicalTrialStudy): Trial {
